...
export default function ClaimReviewQueue({ claims, total, page, pageSize, statusFilter }: { claims: ClaimRow[]; total: number; page: number; pageSize: number; statusFilter: string }) {
  ...
  return (
    <div className="space-y-7" data-testid="internal-claims-queue">
      ...
      <div className="flex flex-wrap gap-2 text-[11px]"><span className="lux-card px-3 py-1.5"><span className="text-slate-500">Baru: </span><span className="font-semibold text-slate-900">{summary.pending}</span></span><span className="lux-card px-3 py-1.5"><span className="text-slate-500">Diperiksa: </span><span className="font-semibold text-slate-900">{summary.review}</span></span><span className="lux-card px-3 py-1.5"><span className="text-slate-500">Disetujui: </span><span className="font-semibold text-slate-900">{summary.approved}</span></span></div>
