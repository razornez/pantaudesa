...
import { getFilteredDiffEntries, getOrderedDiffSections, type DiffFilter } from "./diff-theatre";
  ...
  if (!diff) {
    return (
      ...
      >
        ...
        <p className="text-sm text-slate-500">Pilih desa target untuk membandingkan hasil dengan data yang ada.</p>
      </section>
    );
  }
  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-end justify-between gap-4 px-4 pb-4 pt-6 sm:px-7">
        <div>
          <p className="eyebrow mb-1.5 text-indigo-600">Diff · ujung tombak review</p>
          <h2 className="text-[22px] font-semibold leading-tight text-slate-900 sm:text-[28px]" style={{ letterSpacing: "-0.028em" }}>
            {changeCount > 0 ? (
              <>
                {changeCount} <span className="font-normal text-slate-400">field akan berubah di halaman desa</span>
              </>
            ) : (
              <span className="font-normal text-slate-400">Tidak ada perubahan dari data saat ini</span>
            )}
          </h2>
          <p className="mt-1.5 text-[12px] text-slate-500">Hijau = baru · ungu = diperbarui · merah = hapus · abu = sama</p>
        </div>
      ...
      {filteredEntries.length === 0 ? (
        ...
        orderedSections.map(([sectionName, sectionEntries]) => {
          const sectionChangedCount = sectionEntries.filter((entry) => entry.deltaType !== "unchanged").length;
          return (
            <div key={sectionName} className="border-t border-slate-100 px-3 pb-2 first:border-t-0">
              <div className="flex items-center justify-between px-5 pb-1.5 pt-4">
                <p className="text-[11px] font-semibold uppercase tracking-[0.16em] text-slate-700">
                  {sectionName}
                  <span className="ml-1 font-normal text-slate-400">· {sectionEntries.length} field</span>
                </p>
                {sectionChangedCount > 0 && (
                  <span className="text-[10.5px] tabular-nums text-slate-500">{sectionChangedCount} berubah</span>
                )}
              </div>
