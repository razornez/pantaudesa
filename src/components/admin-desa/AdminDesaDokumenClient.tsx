...
function DocCard({ doc, canApprove, onApprove, onPreview, busyId }: {
  doc: DocRow;
  canApprove: boolean;
  ...
  return (
    <article className="lux-card t-spring p-4 sm:p-5 space-y-3">
      <div className="flex items-start justify-between gap-2">
        ...
        <span className={`inline-flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-full shrink-0 ${status.cls}`}>{status.label}</span>
      </div>

      <div className="flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-slate-600">
        <span>{doc.category}</span><span className="text-slate-300">·</span><span>{uploaderName}</span><span className="text-slate-300">·</span>
      ...
      <div className="flex flex-wrap gap-2">
        <button type="button" onClick={() => onPreview(doc.id)} disabled={busyId === doc.id} className="btn-lux btn-lux-ghost text-xs">
          <ExternalLink size={11} aria-hidden /> {COPY.actions.preview}
        </button>
        {canApprove && doc.status === "WAITING_VERIFIED_APPROVAL" && (
          <button type="button" onClick={() => onApprove(doc.id)} disabled={busyId === doc.id} className="btn-lux btn-lux-success text-xs">
...
function UploadForm({ categories, maxFileSizeMB, maxFilesPerUpload, allowedMimeTypes, storageStatus, onUploaded, onNotify }: {
  categories: ReadonlyArray<{ value: string; label: string }>;
  maxFileSizeMB: number;
  maxFilesPerUpload: number;
  allowedMimeTypes: string[];
  ...
  function reset() {
    setTitle(""); setCategory(categories[0]?.value ?? ""); setFiles([]); setAck(false);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (!storageStatus.configured) {
      ...
      const detail = missing ? COPY.upload.missingStorageDetail(missing) : invalid ? COPY.upload.invalidStorageDetail(invalid) : "Hubungi admin PantauDesa.";
    ...
    for (const file of files) {
      if (file.size > maxFileSizeMB * 1024 * 1024) { onNotify(COPY.upload.fileTooLargeError(file.name, maxFileSizeMB), "error"); return; }
      if (!allowedMimeTypes.includes(file.type)) { onNotify(COPY.upload.fileTypeError(file.name), "error"); return; }
    }
    ...
    try {
      ...
      fd.append("title", title); fd.append("category", category); fd.append("responsibilityAck", "true");
      const res = await fetch("/api/admin-claim/documents/upload", { method: "POST", body: fd });
      const data = await res.json();
      if (!res.ok) {
        const detail = data.detail ? ` (${data.detail})` : "";
        onNotify(`${data.error ?? COPY.upload.uploadFailed}${detail}`, "error");
        return;
      }
      const persisted = Array.isArray(data.documents) ? data.documents.length : 0;
      if (persisted === 0) { onNotify(COPY.upload.serverNoConfirmation, "error"); return; }
      onNotify(COPY.upload.uploadSuccess(persisted), "success");
      reset(); onUploaded();
    } catch {
      onNotify(COMMON_COPY.connectionError, "error");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <form data-testid="document-upload-form" onSubmit={handleSubmit} className="lux-panel p-5 sm:p-6 space-y-4">
      <div className="space-y-1">
        ...
        <h2 className="text-[18px] sm:text-[20px] font-semibold text-slate-900 tracking-tight">{COPY.upload.title}</h2>
        <p className="text-xs text-slate-500">{COPY.upload.helper(maxFileSizeMB, maxFilesPerUpload, formats)}</p>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        <div>
          ...
          <input type="text" value={title} maxLength={200} onChange={(e) => setTitle(e.target.value)} placeholder={COPY.upload.titlePlaceholder} className="field-lux text-sm" required />
        </div>
        <div>
        ...
        <div className="sm:col-span-2">
          ...
          <input ref={fileInputRef} type="file" multiple accept={allowedMimeTypes.join(",")} onChange={handleFileChange} className="block w-full text-sm text-slate-700 file:mr-3 file:rounded-xl file:border-0 file:bg-indigo-50 file:text-indigo-700 file:px-4 file:py-2 file:font-semibold file:cursor-pointer" required />
        </div>
      </div>
      ...
      <label className="flex items-start gap-2 rounded-xl bg-slate-50 px-3 py-2.5 shadow-[inset_0_0_0_1px_rgba(15,23,42,0.05)] text-xs cursor-pointer">
        ...
        <span className="text-slate-700 leading-relaxed">{COPY.upload.responsibility}</span>
      </label>
      <button type="submit" disabled={loading || !storageStatus.configured} className="btn-lux btn-lux-primary w-full text-sm">
        {loading ? COPY.upload.uploading : <><Upload size={13} aria-hidden /> {files.length > 1 ? COPY.upload.uploadMultipleDocuments(files.length) : COPY.upload.uploadDocument}</>}
      </button>
    </form>
  );
}
...
export default function AdminDesaDokumenClient(props: Props) {
  ...
  async function handleApprove(id: string) {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) { toast(data.error ?? COMMON_COPY.genericFailed, "error"); return; }
      toast(COPY.actions.forwardedToPantauDesa, "success");
      router.refresh();
    } catch { toast(COMMON_COPY.connectionError, "error"); } finally { setBusyId(null); }
  }
  ...
  async function handlePreview(id: string) {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok || !data.signedUrl) { toast(data.error ?? COMMON_COPY.genericFailed, "error"); return; }
    ...
    <div className="space-y-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="flex flex-wrap gap-1.5">
          <span className="pill-info rounded-full px-2.5 py-0.5 text-[10px] font-semibold">{summary.total} {COPY.summary.documents}</span>
          {summary.waiting > 0 && <span className="pill-warn rounded-full px-2.5 py-0.5 text-[10px] font-semibold">{summary.waiting} {COPY.summary.waiting.toLowerCase()}</span>}
          {summary.processing > 0 && <span className="pill-info rounded-full px-2.5 py-0.5 text-[10px] font-semibold">{summary.processing} {COPY.status.PROCESSING.toLowerCase()}</span>}
          {summary.published > 0 && <span className="pill-ok rounded-full px-2.5 py-0.5 text-[10px] font-semibold">{summary.published} {COPY.summary.published.toLowerCase()}</span>}
        </div>
      </div>
      <div className="flex flex-wrap gap-2 text-[11px]">
        <span className="lux-card px-3 py-1.5"><span className="text-slate-500">{COPY.summary.total}: </span><span className="font-semibold text-slate-900">{summary.total}</span></span>
        <span className="lux-card px-3 py-1.5"><span className="text-slate-500">{COPY.summary.waiting}: </span><span className="font-semibold text-slate-900">{summary.waiting}</span></span>
        <span className="lux-card px-3 py-1.5"><span className="text-slate-500">{COPY.summary.published}: </span><span className="font-semibold text-slate-900">{summary.published}</span></span>
      </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center gap-2"><FolderKanban size={14} className="text-indigo-600" aria-hidden /><p className="text-[11px] font-semibold text-slate-600">{COPY.list.title}</p></div>
        {props.documents.length === 0 ? <div className="lux-card p-8 text-center space-y-2"><FileText size={22} className="mx-auto text-slate-300" aria-hidden /><p className="text-sm text-slate-500">{COPY.list.empty}</p></div> : <div className="grid gap-3 sm:grid-cols-2">{props.documents.map((doc) => <DocCard key={doc.id} doc={doc} canApprove={canApprove} onApprove={handleApprove} onPreview={handlePreview} busyId={busyId} />)}</div>}
      </section>
