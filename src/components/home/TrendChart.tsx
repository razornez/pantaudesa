...
export default function TrendChart({ data }: Props) {
  return (
    <div className="bg-white rounded-2xl p-5 border border-slate-100 shadow-sm">
      <div className="mb-4">
        <h2 className="text-base font-semibold text-slate-800">Tren Penyerapan Anggaran</h2>
        <p className="text-xs text-slate-500 mt-0.5">Anggaran vs Realisasi sepanjang tahun 2024</p>
      </div>
      <ResponsiveContainer width="100%" height={250}>
        <AreaChart data={data} margin={{ top: 5, right: 10, left: 0, bottom: 0 }}>
          ...
          <YAxis tickFormatter={(v) => `${(v / 1_000_000_000).toFixed(0)}M`} tick={{ fontSize: 10, fill: "#94A3B8" }} axisLine={false} tickLine={false} width={38} />
          <Tooltip
          ...
          />
          <Legend formatter={(v) => (v === "anggaran" ? "Anggaran" : "Realisasi")} wrapperStyle={{ fontSize: 12 }} />
