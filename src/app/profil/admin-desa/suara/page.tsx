  ...
  return (
    <div className="space-y-5">
      <header className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        ...
        <Link href={`/desa/${ctx.desa.id}/suara`} className="btn-lux btn-lux-ghost w-full sm:w-auto text-sm">
      ...
      <section className="grid grid-cols-2 gap-2 sm:grid-cols-4">
        <div className="metric-card p-3 sm:p-4">
          <p className="metric-label text-[10px]">{COPY.summary.total}</p>
          <p className="metric-value text-[1.25rem] sm:text-[1.5rem]">{summary.total}</p>
          <p className="metric-note text-[10px]">{COPY.summary.publicInput}</p>
        </div>
        <div className="metric-card p-3 sm:p-4">
          <p className="metric-label text-[10px]">{COPY.summary.open}</p>
          <p className="metric-value text-[1.25rem] sm:text-[1.5rem]">{summary.open}</p>
          <p className="metric-note text-[10px]">{COPY.summary.unresolved}</p>
        </div>
        <div className="metric-card p-3 sm:p-4">
          <p className="metric-label text-[10px]">{COPY.summary.replies}</p>
          <p className="metric-value text-[1.25rem] sm:text-[1.5rem]">{summary.replies}</p>
          <p className="metric-note text-[10px]">{COPY.summary.discussions}</p>
        </div>
        <div className="metric-card p-3 sm:p-4">
          <p className="metric-label text-[10px]">{COPY.summary.helpful}</p>
          <p className="metric-value text-[1.25rem] sm:text-[1.5rem]">{summary.helpful}</p>
          <p className="metric-note text-[10px]">{COPY.summary.citizenResponse}</p>
        </div>
      </section>

      <div className="notice-card notice-info text-sm leading-relaxed">
        <p className="font-semibold">{COPY.noticeTitle}</p>
        <p className="mt-1 opacity-90">{COPY.noticeBody}</p>
      </div>

      {voices.length === 0 ? (
        <div className="lux-card p-8 text-center text-sm text-slate-500">
          {COPY.empty}
        </div>
      ) : (
              ...
              <li key={v.id} className="lux-card t-spring lift hover:shadow-lux-hover p-4 sm:p-5 space-y-3">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  <div className="grid grid-cols-3 gap-1.5 sm:min-w-[168px]">
                    <div className="rounded-xl bg-slate-50 px-2.5 py-2 text-center shadow-[inset_0_0_0_1px_rgba(15,23,42,0.05)]">
                      <p className="text-[10px] text-slate-500">{COPY.summary.replies}</p>
                      <p className="text-sm font-semibold text-slate-900 mt-0.5">{v._count.replies}</p>
                    </div>
                    <div className="rounded-xl bg-slate-50 px-2.5 py-2 text-center shadow-[inset_0_0_0_1px_rgba(15,23,42,0.05)]">
                      <p className="text-[10px] text-slate-500">{COPY.summary.helpful}</p>
                      <p className="text-sm font-semibold text-slate-900 mt-0.5">{v._count.helpfuls}</p>
                    </div>
                    <div className="rounded-xl bg-slate-50 px-2.5 py-2 text-center shadow-[inset_0_0_0_1px_rgba(15,23,42,0.05)]">
                      <p className="text-[10px] text-slate-500">Vote</p>
                      <p className="text-sm font-semibold text-slate-900 mt-0.5">{v._count.votes}</p>
                    </div>
                  </div>
                </div>
