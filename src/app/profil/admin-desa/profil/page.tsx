          ...
          <div className="space-y-5">
            ...
            <dl className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
              <div className="metric-card p-4">
                <dt className="metric-label">{COPY.metrics.desa}</dt>
              ...
              <div className="metric-card p-4">
                <dt className="metric-label">{COPY.metrics.location}</dt>
                <dd className="mt-2 text-slate-800">{ctx.desa.provinsi}</dd>
                <p className="metric-note">{COPY.metrics.workArea}</p>
              </div>
              <div className="metric-card p-4">
                <dt className="metric-label">{COPY.metrics.inviteAccepted}</dt>
                <dd className="mt-2 text-slate-800">
                  {ctx.member.acceptedAt ? new Date(ctx.member.acceptedAt).toLocaleDateString("id-ID", { dateStyle: "long" }) : COPY.metrics.noInvite}
                </dd>
              </div>
              <div className="metric-card p-4">
                <dt className="metric-label">{COPY.metrics.nextRenewal}</dt>
                <dd className={`mt-2 font-medium ${ctx.renewal.state === "OVERDUE" ? "text-rose-700" : ctx.renewal.state === "URGENT" || ctx.renewal.state === "DUE_SOON" ? "text-amber-700" : "text-slate-800"}`}>
                  {ctx.member.renewalDueAt ? new Date(ctx.member.renewalDueAt).toLocaleDateString("id-ID", { dateStyle: "long" }) : COPY.metrics.noSchedule}
                </dd>
              </div>
            </dl>
          </div>
          ...
          <aside className="space-y-3">
            <div className="notice-card notice-info">
              <div className="flex items-start gap-3">
                <span className="mt-0.5 inline-flex h-10 w-10 items-center justify-center rounded-2xl bg-indigo-100 text-indigo-700"><ShieldCheck size={18} aria-hidden /></span>
            ...
            <div className="notice-card notice-ok">
              <div className="flex items-start gap-3">
                <span className="mt-0.5 inline-flex h-10 w-10 items-center justify-center rounded-2xl bg-emerald-100 text-emerald-700"><Sparkles size={18} aria-hidden /></span>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <div className="lux-card p-5 sm:p-6 space-y-4">
          <div className="flex items-center gap-3">
            <span className="inline-flex h-10 w-10 items-center justify-center rounded-2xl bg-indigo-50 text-indigo-700"><Users size={18} aria-hidden /></span>
            <div><p className="eyebrow text-[10px]">{COPY.access.eyebrow}</p><h2 className="text-[18px] font-semibold text-slate-900 mt-1">{COPY.access.title}</h2></div>
          </div>
          <ul className="space-y-2.5 text-sm text-slate-700 leading-relaxed">
            {(isVerified ? COPY.access.verified : COPY.access.limited).map((item) => <li key={item}>{item}</li>)}
          </ul>
        </div>

        <div className="lux-card p-5 sm:p-6 space-y-4">
          <div className="flex items-center gap-3">
            <span className="inline-flex h-10 w-10 items-center justify-center rounded-2xl bg-amber-50 text-amber-700"><CalendarClock size={18} aria-hidden /></span>
            <div><p className="eyebrow text-[10px]">{COPY.limits.eyebrow}</p><h2 className="text-[18px] font-semibold text-slate-900 mt-1">{COPY.limits.title}</h2></div>
          </div>
          <ul className="space-y-2.5 text-sm leading-relaxed">
            {(isVerified ? COPY.limits.verified : COPY.limits.limited).map((item) => <li key={item} className={isVerified ? "text-slate-700" : "text-rose-700"}>{item}</li>)}
          </ul>
        </div>
      </section>

      {isVerified && (ctx.renewal.state === "URGENT" || ctx.renewal.state === "OVERDUE") && (
        <section className={`notice-card ${ctx.renewal.state === "OVERDUE" ? "notice-danger" : "notice-warn"}`} role="alert">
          <p className="eyebrow text-[10px]" style={{ color: ctx.renewal.state === "OVERDUE" ? "#9F1239" : "#B45309" }}>{COPY.renewalAlert.eyebrow}</p>
          <h2 className="display text-[20px] font-semibold mt-2">{ctx.renewal.state === "OVERDUE" ? COPY.renewalAlert.overdueTitle : COPY.renewalAlert.urgentTitle}</h2>
          <p className="text-sm mt-2 leading-relaxed opacity-90 max-w-3xl">{COPY.renewalAlert.body}</p>
        </section>
      )}
    </div>
  );
