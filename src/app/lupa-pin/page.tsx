...
function LupaPinInner() {
  ...
  const handleNewPin = async (e: React.SyntheticEvent) => {
    e.preventDefault();
    setErr("");
    if (!/^\d{4}$/.test(pin))    { setErr("PIN harus 4 digit angka."); return; }
      ...
      <div className="max-w-sm w-full bg-white rounded-3xl border border-slate-100 shadow-sm p-8 space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            ...
            {step === "otp"    && `Kode 4 digit dikirim ke ${email}.`}
        ...
        {step === "newpin" && (
          <form onSubmit={handleNewPin} className="space-y-4">
            <div>
              <label className="text-xs font-semibold text-slate-600 block mb-1">PIN Baru (4 digit)</label>
              <div className="relative">
